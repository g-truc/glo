...
public:
	...
	void loadTexture(std::string fileName, bool forceLinearTiling)
	{
		...
		vkGetPhysicalDeviceFormatProperties(physicalDevice, static_cast<VkFormat>(tex2D.format()), &formatProperties);
		...
		if (useStaging)
		{
			...
			imageCreateInfo.format = static_cast<VkFormat>(tex2D.format());
		...
		view.format = static_cast<VkFormat>(tex2D.format());
	...
	void prepare()
	{
		...
		loadTexture(
			getAssetPath() + "textures/kueken7_rgb_dxt1_srgb.ktx", 
			false);
