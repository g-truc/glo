...
{
	glo::GetCurrentContext()->draw(Count, InstanceCount, FirstVertex, BaseInstance);
}

void gl5DrawIndexed(uint32_t Count, uint32_t InstanceCount, uint32_t FirstElement, int32_t BaseVertex, uint32_t BaseInstance)
{
	glo::GetCurrentContext()->drawIndexed(Count, InstanceCount, FirstElement, BaseVertex, BaseInstance);
}
