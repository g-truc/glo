	...
	public:
		...
		*/
		void loadTexture(std::string filename, VkFormat format, VulkanTexture *texture, bool forceLinear = false, VkImageUsageFlags imageUsageFlags = VK_IMAGE_USAGE_SAMPLED_BIT)
		{
			...
			texture->width = static_cast<uint32_t>(tex2D.extent(0).x);
			texture->height = static_cast<uint32_t>(tex2D.extent(0).y);
			...
			if (useStaging)
			{
				...
				for (uint32_t i = 0; i < texture->mipLevels; i++)
				{
					...
					bufferCopyRegion.imageExtent.width = static_cast<uint32_t>(tex2D.extent(i).x);
					bufferCopyRegion.imageExtent.height = static_cast<uint32_t>(tex2D.extent(i).y);
					bufferCopyRegion.imageExtent.depth = 1;
					bufferCopyRegion.bufferOffset = offset;

					bufferCopyRegions.push_back(bufferCopyRegion);

					offset += static_cast<uint32_t>(tex2D.size(i));
				}
			...
			else
			{
				...
				memcpy(data, tex2D.data(0, 0, subRes.mipLevel), tex2D.size(subRes.mipLevel));
		...
		void loadCubemap(std::string filename, VkFormat format, VulkanTexture *texture, VkImageUsageFlags imageUsageFlags = VK_IMAGE_USAGE_SAMPLED_BIT)
		{
			...
			for (uint32_t face = 0; face < 6; face++)
			{
				for (uint32_t level = 0; level < texture->mipLevels; level++)
				{
					...
					bufferCopyRegion.imageSubresource.layerCount = 1;
					bufferCopyRegion.imageExtent.width = static_cast<uint32_t>(texCube.extent(level).x);
					bufferCopyRegion.imageExtent.height = static_cast<uint32_t>(texCube.extent(level).y);
					bufferCopyRegion.imageExtent.depth = 1;
					bufferCopyRegion.bufferOffset = offset;
					...
					offset += texCube[face].size(level);
				}
			}
		...
		void loadTextureArray(std::string filename, VkFormat format, VulkanTexture *texture, VkImageUsageFlags imageUsageFlags = VK_IMAGE_USAGE_SAMPLED_BIT)
		{
			...
			for (uint32_t layer = 0; layer < texture->layerCount; layer++)
			{
				for (uint32_t level = 0; level < texture->mipLevels; level++)
				{
					...
					bufferCopyRegion.imageSubresource.layerCount = 1;
					bufferCopyRegion.imageExtent.width = static_cast<uint32_t>(tex2DArray.extent(level).x);
					bufferCopyRegion.imageExtent.height = static_cast<uint32_t>(tex2DArray.extent(level).y);
					bufferCopyRegion.imageExtent.depth = 1;
					bufferCopyRegion.bufferOffset = offset;
